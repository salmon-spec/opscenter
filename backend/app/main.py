...
def delete_server(server_id: str):
    with get_db() as db:
        ...
        try:
            ...
            if server_id in groups_data:
                del groups_data[server_id]
...
def _sync_port_driven_scan(srv, db, scan_data):
    """Port-driven scan: use (server_id, port, proto) as unique key.
    
    Replaces the old _sync_agent_ports_and_systemd which used container_name
    as dedup key with various prefixes (port:, systemd:, etc.) causing duplicates.
    Now: one logical service per (port, proto) combination.
    """
    ...
    container_port_map = {}  # port -> container info
    for c in containers:
        for p in c.get("ports", []):
            hp = p.get("host_port")
            if hp and str(hp).isdigit():
                container_port_map[int(hp)] = c
    
    added = 0
    updated = 0
    
    # --- Process containers with port-driven dedup ---
    # Each container gets a (server_id, port, proto) key
    for c in containers:
        name = c.get("name", "")
        image = c.get("image", "")
        ...
        svc_url = _build_svc_url_for_remote(name, srv.host, c) if '_build_svc_url_for_remote' in dir() else ""
        ...
        # Get primary port from container
        primary_port = None
        for p in c.get("ports", []):
            hp = p.get("host_port")
            if hp and str(hp).isdigit():
        ...
        # Dedup by (server_id, port, proto)
        existing = db.query(Service).filter(
            Service.server_id == srv.id,
            Service.port == primary_port,
            Service.proto == "tcp",
        ).first()
        ...
        if existing:
            ...
            for field, val in [("name", svc_name), ("url", svc_url), ("category", svc_category),
                               ...
                               ("container_name", name), ("host_ip", srv.host)]:
    ...
    # --- Process listening ports not covered by containers ---
    ...
    for p in ports:
        ...
        if port_num in container_port_map:
            continue
        # Skip localhost-only docker-proxy ports
    ...
    for process, port_list in port_services.items():
        ...
        # Dedup by (server_id, port, proto) - the new unique key
        existing = db.query(Service).filter(
            Service.server_id == srv.id,
        ...
        if existing:
            ...
            for field, val in [("name", svc_name), ("url", svc_url), ("category", svc_category),
                               ...
                               ("container_name", f"port:{process}:{port_num}"), ("host_ip", srv.host)]:
                if val is not None and getattr(existing, field) != val:
                    if field == 'url' and existing.url_overridden:
                        continue
                    setattr(existing, field, val)
            ...
            svc = Service(
                ...
                container_name=f"port:{process}:{port_num}",
                ports=str(port_num),
    ...
    # --- Process systemd services (no port, supplementary) ---
    for svc_info in systemd_services:
        ...
        # Skip systemd if already covered by a port-based or container service
        ...
        # Systemd services use container_name for dedup (no port)
