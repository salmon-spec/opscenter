...
def plaza_owned_service_ids(db) -> set:
    ...
    for service in db.query(Service).filter(
        ...
        # Keep this guard because lightweight test/fake query objects may not
        # evaluate SQLAlchemy filter expressions.
    ...
    with get_db() as db:
        ...
        for service in manual_services:
            if getattr(service, "source", None) != ServiceSource.manual.value:
                continue
            url = (service.url or "").rstrip("/")
            if not url.startswith(("http://", "https://")) or url in catalog_urls:
                continue
