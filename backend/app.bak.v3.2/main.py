...
# === Group CRUD (per spec: /api/v2/groups ...) ===
# These endpoints persist to groups.json and match the frontend contract exactly.
...
def _slugify(name: str) -> str:
    # Generate an ASCII-safe id. Non-ASCII names fall back to a uuid suffix.
...
def create_group(item: GroupCreate):
    """Add a new group. Auto-generate id, push order to last, persist to groups.json."""
    current = _read_groups_json()
...
def update_group(group_id: str, data: GroupUpdate):
    """Update a group: name/color/icon/order."""
    current = _read_groups_json()
    groups = current.get("groups", [])
    found = False
    for g in groups:
...
def delete_group_endpoint(group_id: str):
    """Delete a group. Forbidden if not empty or if 'ungrouped'."""
    if group_id == "ungrouped":
        raise HTTPException(400, "Cannot delete the 'ungrouped' group")
    current = _read_groups_json()
    groups = current.get("groups", [])
    ...
    if not target:
        raise HTTPException(404, "Group not found")
    # Count services still assigned to this group (service map, category map, category-name match)
    svc_map = current.get("serviceGroupMap", {})
    count = sum(1 for v in svc_map.values() if v == group_id)
    c_map = current.get("categoryGroupMap", {})
    count += sum(1 for v in c_map.values() if v == group_id)
    try:
        with get_db() as db:
            cat_count = db.query(Service).filter(Service.category == target.get("name")).count()
        count += cat_count
    except Exception:
        pass
    if count > 0:
        raise HTTPException(400, "group not empty")
...
def move_service_group(data: ServiceGroupMove):
    """Move a service to a group. Persist mapping in groups.json serviceGroupMap."""
    current = _read_groups_json()
    svc_map = current.get("serviceGroupMap", {})
    ...
    _write_groups_json(current)
    return {"ok": True, "serviceKey": data.serviceKey, "groupId": data.groupId}
