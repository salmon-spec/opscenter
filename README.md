...
| Agent | v2.5.1 |
| 版本里程碑 | **v4.7.0**（服务广场持久排序、多账号密码与凭证详情布局） |
...
| Agent | Python 轻量采集器（v2.5.1，轻量系统摘要 / 指标 / 服务发现 / PVE 来宾 Web 服务发现 / 进程管理） |
...
│   │   └── version.py            # 版本号（4.7.0，单一来源）
│   ├── tests/                    # pytest（158 用例）
│   └── requirements.txt
├── agent/
│   ├── opsagent.py               # 远程采集 Agent（v2.5.1）
│   └── scanner.py                # 主机扫描器
