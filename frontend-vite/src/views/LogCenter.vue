...
async function loadLogs(){if(!selectedHostId.value||!status.ready)return;controller?.abort();controller=new AbortController();const active=controller;loading.value=true;error.value='';try{const {start,end}=range();const query={start:start.toISOString(),end:end.toISOString(),source:source.value,service:service.value,search:search.value};const [data,summary]=await Promise.all([api.get(`/servers/${selectedHostId.value}/logs/query`,{...query,limit:limit.value,direction:'backward'},{signal:active.signal}),api.get(`/servers/${selectedHostId.value}/logs/stats`,query,{signal:active.signal})]);entries.value=data.entries||[];Object.assign(stats,summary)}catch(e){if(e.name!=='AbortError')error.value=e.message}finally{if(controller===active)loading.value=false}}
