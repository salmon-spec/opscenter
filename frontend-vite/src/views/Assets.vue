<template>
  <div class="view assets-workbench">
    ...
    <div v-if="hostLoading && !hosts.length" class="loading"><span class="spinner"></span>正在加载主机…</div>
    ...
    <template v-else>
      <section class="card workbench">
        ...
        <div v-if="activeTab === 'containers'" class="tab-panel">
          ...
          <div v-if="containerLoading && !containerHydrated" class="loading"><span class="spinner"></span>正在读取 Docker 容器…</div>
...
const containers = ref([]), containerTotal = ref(0), containerLoading = ref(false), containerHydrated = ref(false), containerFilter = ref('all'), containerSearch = ref(''), selectedContainers = ref(new Set()), operationBusy = ref(false)
...
async function loadContainers(includeStats=false,refresh=false){if(!selectedHostId.value)return;const hostId=selectedHostId.value;containerController?.abort();containerController=new AbortController();const queryKey=encodeURIComponent(`${containerFilter.value}|${containerSearch.value.trim().toLowerCase()}`);const cacheKey=`ops-containers-v2-${hostId}-${queryKey}`;if(!includeStats&&!refresh){try{const cached=JSON.parse(sessionStorage.getItem(cacheKey)||'null');if(cached?.items){containers.value=cached.items;containerTotal.value=cached.total??cached.items.length;containerHydrated.value=true;containerDataTime.value='会话缓存 '+new Date(cached.time).toLocaleTimeString('zh-CN',{hour12:false})}}catch{}}containerLoading.value=true;try{const data=await api.get(`/servers/${hostId}/containers`,{status:containerFilter.value,search:containerSearch.value,page_size:200,include_stats:includeStats,refresh},{signal:containerController.signal});if(hostId!==selectedHostId.value)return;containers.value=data.items||[];containerTotal.value=data.total||0;containerHydrated.value=true;const stamp=Number(data.data_timestamp||Date.now()/1000)*1000;const timing=data.duration_ms!==undefined?` · ${Number(data.duration_ms).toFixed(0)}ms`:'';const source=data.source?` · ${data.source}`:'';containerDataTime.value=new Date(stamp).toLocaleTimeString('zh-CN',{hour12:false})+(data.cached?` · 缓存 ${Number(data.cache_age_seconds||0).toFixed(1)}s`:'')+timing+source;sessionStorage.setItem(cacheKey,JSON.stringify({items:containers.value,total:containerTotal.value,time:stamp}));selectedContainers.value=new Set([...selectedContainers.value].filter(id=>containers.value.some(item=>item.id===id)))}catch(error){if(error.name==='AbortError')return;if(!containerHydrated.value){containers.value=[];containerTotal.value=0}toast(`容器读取失败：${error.message}`,'err')}finally{if(hostId===selectedHostId.value)containerLoading.value=false}}
...
watch(selectedHostId,async()=>{selectedContainers.value=new Set();selectedResources.value=new Set();containers.value=[];containerHydrated.value=false;dockerResourceTabs.forEach(key=>{dockerResources[key]=[];resourceMeta[key]={}});await refreshCurrent()})
