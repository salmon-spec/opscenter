...
const editing=ref(false),saving=ref(false),testing=ref(false),testMessage=ref(''),testOk=ref(false),currentAgentVersion=ref('2.4.0'),upgrading=ref(false),alloyBusy=ref(false),lokiConfigured=ref(false)
...
async function loadAgentVersion(){try{const [agent,alloy]=await Promise.all([api.get('/agents/version'),api.get('/logs/agents/version')]);currentAgentVersion.value=agent.current_version||'2.4.0';lokiConfigured.value=!!alloy.loki_configured}catch{/* 使用内置目标版本 */}}
