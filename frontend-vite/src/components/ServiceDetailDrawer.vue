...
const detail = ref(null), loading = ref(false), editing = ref(false), saving = ref(false), probing = ref(false), hosts = ref([]), revealedPassword = ref('')
let hideTimer = null
const form = reactive({ server_id: '', name: '', category: '', icon: '', entry_url: '', health_url: '', owner: '', documentation_url: '', tagsText: '', description: '', username: '', password: '', login_notes: '', clear_password: false, probe_enabled: true, probe_interval_seconds: 60, probe_timeout_seconds: 4, probe_success_statuses: '200-399,401,403', probe_verify_tls: true })
...
async function startEdit(){try{if(!detail.value)await loadDetail();if(!hosts.value.length)hosts.value=await api.get('/servers');const d=detail.value||props.service||{},p=d.probe_policy||{};Object.assign(form,{server_id:d.server?.id||d.server_id||'',name:d.name||'',category:d.category||'',icon:d.icon||'box',entry_url:d.entry_url||d.url||'',health_url:d.health_url||'',owner:d.owner||'',documentation_url:d.documentation_url||'',tagsText:(d.tags||[]).join(', '),description:d.description||'',username:d.credential_username||'',password:'',login_notes:d.login_notes||'',clear_password:false,probe_enabled:p.enabled!==false,probe_interval_seconds:p.interval_seconds||60,probe_timeout_seconds:p.timeout_seconds||4,probe_success_statuses:p.success_statuses||'200-399,401,403',probe_verify_tls:p.verify_tls!==false});editing.value=true}catch(error){toast(`编辑信息加载失败：${error.message}`,'error')}}
async function saveEdit(){if(!form.name||!form.category||!/^https?:\/\//i.test(form.entry_url)){toast('请填写名称、分类和正确的 HTTP(S) 地址','error');return}saving.value=true;try{const payload={server_id:form.server_id||null,name:form.name,category:form.category,icon:form.icon,entry_url:form.entry_url,health_url:form.health_url,owner:form.owner,documentation_url:form.documentation_url,tags:form.tagsText.split(/[,，]/).map(x=>x.trim()).filter(Boolean),description:form.description,username:form.username,login_notes:form.login_notes,clear_password:form.clear_password,probe_enabled:form.probe_enabled,probe_interval_seconds:form.probe_interval_seconds,probe_timeout_seconds:form.probe_timeout_seconds,probe_success_statuses:form.probe_success_statuses,probe_verify_tls:form.probe_verify_tls};if(form.password)payload.password=form.password;await api.put(`/services/plaza/${encodeURIComponent(plazaKey.value)}`,payload);editing.value=false;await loadDetail();emit('updated',detail.value);toast('服务信息已保存','success')}catch(error){toast(`保存失败：${error.message}`,'error')}finally{saving.value=false}}
async function revealPassword(){if(revealedPassword.value){clearReveal();return}try{const data=await api.post(`/services/plaza/${encodeURIComponent(plazaKey.value)}/credentials/reveal`);revealedPassword.value=data.password||'';detail.value.credential_access_history=[{id:`local-${Date.now()}`,action:'reveal',actor:'admin',ip:'当前访问',created_at:new Date().toISOString()},...(detail.value.credential_access_history||[])];hideTimer=setTimeout(clearReveal,60000)}catch(error){toast(`密码显示失败：${error.message}`,'error')}}
