      ...
      <div v-if="visible" class="drawer-mask" @click.self="closeDrawer">
        <div class="drawer">
          <div class="drawer-head">
            ...
            <div class="head-actions"><button v-if="plazaKey && !editing" class="btn btn-sm" @click="startEdit">编辑信息</button><button v-if="isManual && !editing" class="btn btn-danger btn-sm" @click="deleteService">删除服务</button><button class="btn btn-ghost btn-sm" @click="closeDrawer">✕</button></div>
          </div>
...
const isManual = computed(() => !!(detail.value?.manual || props.service?.manual))
...
async function deleteService(){const id=detail.value?.service_id||props.service?.service_id;if(!id||!confirm(`确认永久删除手动服务「${detail.value?.name||props.service?.name}」？此操作不可恢复。`))return;try{await api.del(`/services/${id}`);toast('手动服务已删除','success');emit('deleted');closeDrawer()}catch(error){toast(`删除失败：${error.message}`,'error')}}
